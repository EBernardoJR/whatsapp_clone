...
export const userChatsFetch = ()=> {

    const { currentUser } = firebase.auth()

    return dispatch => {
        let userLogEmailB64 = b64.encode(currentUser.email)//usuário logado

        firebase.database().ref(`/user_chats/${userLogEmailB64}`)
            .on('value', snapshot => {
                dispatch({
                    type: USER_CHATS_LIST,
